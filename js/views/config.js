...
function galeriaFondos(){
  const actual=localStorage.getItem('hub_fondo')||'ninguno',personal=localStorage.getItem('hub_fondo_personal');
  return `<div class="fondos-grid fondos-grid-manual"><button class="fondo-opcion ${actual==='ninguno'?'on':''}" onclick="fijarFondo('ninguno')"><span class="fondo-mini sin-fondo"></span><strong>Sin fondo</strong>${actual==='ninguno'?'<b>✓</b>':''}</button>${personal?`<button class="fondo-opcion ${actual==='personal'?'on':''}" onclick="fijarFondo('personal')" style="--mini-fondo:url('${personal}')"><span class="fondo-mini"></span><strong>Mi fondo</strong>${actual==='personal'?'<b>✓</b>':''}</button>`:''}</div><div class="fondo-subir"><label class="btn"><input type="file" accept="image/*" hidden onchange="subirFondoPersonal(this)">＋ ${personal?'Cambiar mi fondo':'Agregar mi fondo'}</label>${personal?'<button class="btn dgr" onclick="eliminarFondoPersonal()">Eliminar fondo personal</button>':''}</div>`;
}
