...
function controlPreguntaPublica(q){
  ...
  if(q.tipo==='texto') return `<textarea ${attrs} rows="3"></textarea>`;
...
async function mostrarFormularioPublico(token){
  ...
  document.body.innerHTML=`<div class="pub-page"><div class="pub-brand">${logoNexa(34,'pub')}<div><b>NEXA</b><small>Formulario público</small></div></div>
    <form class="pub-card" id="formPublico" onsubmit="enviarFormularioPublico(event)">
      ...
      `<input id="pubTrampa" tabindex="-1" autocomplete="off" style="position:absolute;left:-9999px" aria-hidden="true">
       ${preguntasPublicas.length?preguntasPublicas.map((q,i)=>`<div class="pub-pregunta"><label for="pub_${q.id}"><span>${i+1}</span>${esc(q.texto)}${q.obligatoria?'<b>*</b>':''}</label>${controlPreguntaPublica(q)}</div>`).join(''):'<div class="pub-ayuda">Este formulario todavía no tiene preguntas.</div>'}
...
function valorPreguntaPublica(q){
  ...
  return document.getElementById(`pub_${q.id}`)?.value?.trim()||'';
}

async function enviarFormularioPublico(e){
  e.preventDefault();if($('#pubTrampa')?.value)return;
  const datos={};let falta='';
  for(const q of preguntasPublicas){const v=valorPreguntaPublica(q);datos[q.id]=v;if(q.obligatoria&&(!v||(Array.isArray(v)&&!v.length))&&!falta)falta=q.texto;}
